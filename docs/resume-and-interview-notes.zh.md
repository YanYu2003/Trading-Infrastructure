...
Phase 5B 增加了 SQLite run-history 只读查询 CLI：
...
- 在进程退出后检查已持久化的 trading run。
- 输出稳定的 CSV-style 文本。
- 查询最终订单状态。
- 查询成交记录和 notional。
- 查询账户快照。
- 查询每个 snapshot 下的 positions。
- 按 snapshot index 过滤 position rows。
- 查询行为保持在交易核心之外。
...
代码位置：`src/mini_trading/app/run_history.py`

Run-history CLI 是 SQLite 持久化层之上的只读审计接口。它让用户可以在交易 run 结束后查看已保存的记录，而不需要重新运行策略，也不会修改 portfolio 状态。
...
- 查询已持久化订单。
- 查询已持久化成交。
- 查询 account snapshots。
- 查询逐 snapshot positions。
...
#### Run-history 查询层增加了什么？

它让已持久化的交易 run 在进程退出后仍然可以被检查。Phase 5B 不只是把数据存进数据库，还提供命令查询 run summary、orders、fills、account snapshots 和逐 snapshot positions。这是审计层，不是交易层。

#### 为什么查询层是只读的？

历史交易记录应该被视为交易引擎产生的事实，不应该由查看工具随意修改。只读 CLI 可以保护 orders、fills 和 account snapshots 的审计语义。
...
### 已完成 Phase 5B：Run History 查询 CLI

目标已完成：已持久化的交易 run 可以通过只读 CLI 检查。
